...
function WeatherCard({ lat, lon }) {
  ...
  const getCustomIcon = (weather) => {
    return `${process.env.PUBLIC_URL}${weatherIcon[weather.toLowerCase()]}`;
  };
          ...
          <div className="right">
            {
              location?.weather && location.weather[0] && (
                <img src={getCustomIcon(location?.weather[0].description)} alt="Weather Icon" />)
            }
          </div>
        </div>
